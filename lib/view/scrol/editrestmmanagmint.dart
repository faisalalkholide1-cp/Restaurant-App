...
class EditRestmManagmint extends StatefulWidget {
  RestaurantModel rest;
  EditRestmManagmint({super.key, required this.rest});

  @override
  State<EditRestmManagmint> createState() => _EditRestmManagmintState();
}

class _EditRestmManagmintState extends State<EditRestmManagmint> {
  ...
  @override
  void initState() {
    name.text = widget.rest.name;
    subName.text = widget.rest.subdomain;
    dateCreat.text = widget.rest.createdAt;
    _startDate = _dateFormatter.parse(widget.rest.subscriptionStartDate);
    _endDate = _dateFormatter.parse(widget.rest.subscriptionEndDate);
    super.initState();
  }
  ...
  sveEdit() async {
    var response = await Apiservice.postRequest(LinkApi.restEdit, {
      "id": widget.rest.id,
      "name": name.text,
